...
struct l_theta
{
  ...
  void create(int topology[HIDDEN_LAYERS+2]){ //HIDDEN_LAYERS+ INPUT LAYER + OUTPUT LAYER
    ...
    this->DW = new double*[i_size*numthreads];
    for(int i = 0; i < i_size*numthreads; ++i){
...
struct bpnet
{
  ...
  void backward_training(int m,int myrank){
    ...
    for (int j = 0; j < this->theta[HIDDEN_LAYERS].o_size; j++) {
      ...
      this->theta[HIDDEN_LAYERS].DW[0+numthreads*myrank][j]=prediccion-Y_TRAIN[m][j];
      //printf("Actualizacion:%lf\n",this->theta[HIDDEN_LAYERS].DW[0][j]);
      this->theta[HIDDEN_LAYERS].DW[0+numthreads*myrank][j]*=dsigmoid(prediccion);
      //printf("Actualizacion:%lf\n",dsigmoid(prediccion));
      //printf("Bias resultado:%lf\n",this->theta[HIDDEN_LAYERS].DW[0][j]);
      for (int i = 1; i < this->theta[HIDDEN_LAYERS].i_size; i++) {
        this->theta[HIDDEN_LAYERS].DW[i+numthreads*myrank][j]=this->theta[HIDDEN_LAYERS].DW[0][j]*this->in_forward[HIDDEN_LAYERS][i-1];
    ...
    if(HIDDEN_LAYERS>0){
      ...
      for (int l = HIDDEN_LAYERS-1; l >-1; l--) {
        for (int j = 0; j < this->theta[l].o_size; j++) {
          ...
          for (int k = 0; k < this->theta[l+1].o_size; k++) {
            sum+=this->theta[l+1].W[j+1][k]*this->theta[l+1].DW[0+numthreads*myrank][k];
          }
          //printf("SUMA: %lf\n",sum);
          this->theta[l].DW[0+numthreads*myrank][j]=sum*dsigmoid(this->in_forward[l+1][j]);
          //printf("Dsigmoid: %lf\n",dsigmoid(this->in_forward[l+1][j]));
          //printf("Bias %u: %lf\n",j,this->theta[l].DW[0][j]);
          for (int i = 1; i < this->theta[l].i_size; i++) {
            this->theta[l].DW[i+numthreads*myrank][j]=this->theta[l].DW[0+numthreads*myrank][j]*this->in_forward[l][i-1];
      ...
      for (int l = 0; l < HIDDEN_LAYERS+1; l++) {
        //printf("CAPA %u\n",l);
        for (int i = 0; i < this->theta[l].i_size*numthreads; i++) {
          for (int j = 0; j < this->theta[l].o_size; j++) {
...
#   pragma omp parallel for num_threads(numthreads) reduction(+: error_epoch)
    for (int m = 0; m < TRAIN_DSIZE; m++) {
      int myrank=omp_get_thread_num();
      net.forward(m,myrank);
      net.backward_training(m,myrank);
    }
    if (ep%100==0) {
